...
//                             {/* <p className="text-gray-600 text-center">Votes: {party.votes}</p> */}
//                             <button
//                                 onClick={() => setSelectedParty(party.name)}
...
const ElectionDetailsPage = () => {
    ...
    useEffect(() => {
        const fetchElectionDetails = async () => {
            try {
                const response = await axios.get(`https://i-vote-backend-main.vercel.app/election/${id}`);
                const electionData = response.data;
                setElection(electionData);
    ...
    const handleVote = async () => {
        try {
            const response = await axios.post(`https://i-vote-backend-main.vercel.app/cast/${id}`, { party: selectedParty }, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
